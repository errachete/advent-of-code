...
int main() 
{
    ...
    // Construct vector of possible entries
    vector<pair<ii, char>> entries;
    for (int i = 0; i < cont.size(); ++i)
    {
        entries.emplace_back(make_pair(i,0), 'R');
        entries.emplace_back(make_pair(i,cont[i].size()-1), 'L');
    }
    for (int j = 0; j < cont[0].size(); ++j)
    {
        entries.emplace_back(make_pair(0,j), 'D');
        entries.emplace_back(make_pair(cont.size()-1, j), 'U');
    }

    // Repeat problem for each entry, and store max result
    int max = 0;
    for (auto start : entries)
    {
        ...
        max = visit.size() > max ? visit.size() : max;
    }

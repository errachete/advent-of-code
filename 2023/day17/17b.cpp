...
int dijkstra(const vector<vector<int>> & fac)
{
    ...
    while (!pending.empty())
    {
        ...
        if (currNode.row == fac.size()-1 && currNode.col == fac.back().size()-1 && currNode.cons >= 3)
        ...
        for (int i = 0; i < dCoords.size(); ++i)
        {
            ...
            if (nextNode.row >= 0 && nextNode.row < fac.size() &&
                ...
                ((nextNode.direc != currNode.direc && currNode.cons >= 3) ||  
                (nextNode.direc == currNode.direc && nextNode.cons < 10)) && 
                nextNode.direc != opDir(currNode.direc))
            {
